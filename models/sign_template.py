...
class SignItemType(models.Model):
    ...
    sign_type = fields.Selection(string="Type", selection=[
        ('res.partner', 'Partner'),
    ...
    def _check_auto_field_exists(self):
        if self.sign_type == 'res.partner':
            partner = self.env['res.partner'].browse(self.env.user.partner_id.id)
        elif self.sign_type == 'hr.employee':
            partner = self.env['hr.employee'].search([], order='id', limit=1)
